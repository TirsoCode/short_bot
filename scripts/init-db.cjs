...
async function initDb() {
  ...
  if (existing.length === 0 || existing[0].values.length === 0) {
    run(`INSERT INTO settings (id, github_owner, github_repo, github_branch, github_paths, github_token, youtube_client_id, youtube_client_secret, sync_interval_minutes, max_short_duration, video_width, video_height, video_fps) VALUES ('default', '', '', 'main', '["videos", "screenshots"]', '', '', '', 30, 30, 1080, 1920, 30)`);
  }
