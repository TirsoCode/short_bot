...
export const settingsQueries = {
  ...
  async update(data: any) { const sets: string[] = []; const vals: any[] = []; Object.entries(data).forEach(([k, v]) => { if (v === undefined) return; const col = k === 'githubOwner' ? 'github_owner' : k === 'githubRepo' ? 'github_repo' : k === 'githubBranch' ? 'github_branch' : k === 'githubPaths' ? 'github_paths' : k === 'githubToken' ? 'github_token' : k === 'youtubeClientId' ? 'youtube_client_id' : k === 'youtubeClientSecret' ? 'youtube_client_secret' : k === 'youtubeRefreshToken' ? 'youtube_refresh_token' : k === 'syncIntervalMinutes' ? 'sync_interval_minutes' : k === 'maxShortDuration' ? 'max_short_duration' : k === 'videoWidth' ? 'video_width' : k === 'videoHeight' ? 'video_height' : k === 'videoFps' ? 'video_fps' : k === 'mediaPaths' ? 'media_paths' : k === 'autoShortsPerDay' ? 'auto_shorts_per_day' : k === 'autoPublish' ? 'auto_publish' : k === 'autoRuns' ? 'auto_runs' : k === 'styleJson' ? 'style_json' : k; const val = (typeof v === 'object' && v !== null) ? JSON.stringify(v) : v; sets.push(`${col} = ?`); vals.push(val); }); if (sets.length) { sets.push('updated_at = CURRENT_TIMESTAMP'); await run(`UPDATE settings SET ${sets.join(', ')} WHERE id = ?`, [...vals, 'default']); } },
};
